...
def get_parser():
    parser = argparse.ArgumentParser(description="Run autoRIFT to find pixel offsets for two HLS images")
    parser.add_argument("img1_date", type=str, help="date of first HLS image ('YYYY-mm-dd')")
    parser.add_argument("img2_date", type=str, help="date of second HLS image ('YYYY-mm-dd')")
    return parser
...
def main():
    ...
    # download HLS images
    img1_ds, img2_ds = download_hls(args.img1_date, args.img2_date, aoi)
    # grab red band only
    img1 = img1_ds.B04.squeeze().values
    img2 = img2_ds.B04.squeeze().values
    
    # scale search limit with temporal baseline assuming max velocity 1000 m/yr (100 px/yr)
    search_limit_x = search_limit_y = round(((((img2_ds.time.isel(time=0) - img1_ds.time.isel(time=0)).dt.days)*33)/365.25).item())
    ...
    ds.veloc_x.rio.to_raster(f'HLS_{args.img1_date}_{args.img2_date}_veloc_x.tif')
    ds.veloc_y.rio.to_raster(f'HLS_{args.img1_date}_{args.img2_date}_veloc_y.tif')
